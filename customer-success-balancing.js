...
function customerSuccessBalancing(customerSuccess, customers, customerSuccessAway) {
  ...
  for (let i = 0; i < customerSuccess.length; i++) {
    const filterCustomers = customers.filter(
      (customer) =>
        customerSuccess[i].score >= customer.score && !filteredCustomers.has(customer.id)
    );

    ranking.push({
      idCustomerSuccess: customerSuccess[i].id,
      len: filterCustomers.length,
      customers: filterCustomers,
    });

    filterCustomers.forEach((customer) => filteredCustomers.add(customer.id));
  }

  ranking.sort((a, b) => (a.len > b.len ? -1 : 1));

  if (ranking[0].len === ranking[1].len) {
    return 0;
  }

  return ranking[0].idCustomerSuccess;
}
test("Scenario 1", () => {
  const css = [
    { id: 1, score: 60 },
